package org.openflexo.technologyadapter.capella.model;
/**
 * 
...
public class TestLoadXXDocuments extends AbstractXXTest {
	...
	public void testInitializeServiceManager() throws Exception {
		instanciateTestServiceManager(CapellaTechnologyAdapter.class);
	}

	@Test
	@TestOrder(3)
	public void testXXLoading() {
		CapellaTechnologyAdapter technologicalAdapter = serviceManager.getTechnologyAdapterService()
				.getTechnologyAdapter(CapellaTechnologyAdapter.class);

		for (FlexoResourceCenter<?> resourceCenter : serviceManager.getResourceCenterService().getResourceCenters()) {
			XXTextResourceRepository<?> workbookRepository = technologicalAdapter.getXXResourceRepository(resourceCenter);
			assertNotNull(workbookRepository);
			Collection<XXTextResource> documents = workbookRepository.getAllResources();
			for (XXTextResource docResource : documents) {
				try {
